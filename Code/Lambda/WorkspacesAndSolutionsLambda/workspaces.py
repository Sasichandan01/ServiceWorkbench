...
def create_workspace(event,context):
    try:
        ...
        response=workspace_table.query(IndexName='CreatedBy-Index', KeyConditionExpression=Key('CreatedBy').eq('user_id'),FilterExpression=Attr('WorkspaceName').eq(body.get('WorkspaceName')))
        ...
        log_activity('CREATE_WORKSPACE', 'Workspace', workspace_id, user_id)
        ...
        if queryParams and queryParams.get('action'):
            ...
            if action not in ['Enable', 'Disable']:
                ...
                if workspace_response.get('WorkspaceStatus') == 'Active':
                    ...
                    log_activity('UPDATE_WORKSPACE', 'Workspace', workspace_id, user_id)
...
def delete_workspace(event,context):
    try:
        ...
        if workspace_response.get('WorkspaceStatus') == 'Inactive':
            ...
            log_activity('DELETE_WORKSPACE', 'Workspace', workspace_id, user_id)
