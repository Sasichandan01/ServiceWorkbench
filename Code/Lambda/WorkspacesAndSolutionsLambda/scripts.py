...
import os
import logging
from Utils.utils import return_response, log_activity
...
SOLUTION_TABLE = os.environ['SOLUTIONS_TABLE']

# Initialize DynamoDB resource and table
dynamodb = boto3.resource('dynamodb')
activity_logs = dynamodb.Table(ACTIVITY_LOGS_TABLE)
solutions_table = dynamodb.Table(SOLUTION_TABLE)

def handle_get(base_prefix,solution_id):
    """
    ...
    try:
        ...
        code_files = list_files(code_prefix, ['.py', '.json'])
...
def handle_post(base_prefix, body,solution_id, workspace_id, user_id):
    """
    ...
    try:
        ...
        if ext == '.py':
            ...
            if content_type != 'application/json':
                LOGGER.warning("[POST] Forcing ContentType to application/json for .json file")
        ...
        try:
            solution_resp = solutions_table.get_item(
                Key={
                    "WorkspaceId": workspace_id,
                    "SolutionId": solution_id
                }
            )
            solution_item = solution_resp.get("Item", {})
            solution_name = solution_item.get("SolutionName", "Unknown")
            LOGGER.info(f"[DDB] Fetched solution name: {solution_name}")
        except Exception as e:
            LOGGER.warning(f"[DDB] Failed to fetch solution name: {e}")
