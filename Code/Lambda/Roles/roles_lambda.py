...
def lambda_handler(event, context):
    ...
    """
    LOGGER.info(
        "IN roles_lambda.lambda_handler, Event received: %s", event
    )
    ...
    try:
        ...
        if (
            ...
            try:
                ...
                LOGGER.info(
                    "IN roles_lambda.lambda_handler, successfully synchronized system roles"
                )
        ...
        if not valid:
            LOGGER.info(
                "IN roles_lambda.lambda_handler, RBAC denied for user %s on %s %s: %s",
                user_id, method, resource, msg
            )
            ...
            if method == "GET":
                ...
                try:
                    ...
                    while "LastEvaluatedKey" in response:
                        ...
                        items.extend(response.get("Items", []))
                    LOGGER.info(
                        "IN roles_lambda.lambda_handler, scanned %d role items", len(items)
                    )
                except botocore.exceptions.ClientError as e:
                    LOGGER.exception(
                        "IN roles_lambda.lambda_handler, DynamoDB scan failed: %s", e
                    )
                ...
                return paginate_list(
                    "Roles", items, ["Role"], offset, limit, sort_by, sort_order
                )
                ...
                try:
                    ...
                    LOGGER.info(
                        "IN roles_lambda.lambda_handler, Parsed request body JSON"
                    )
                except json.JSONDecodeError as e:
                    LOGGER.exception(
                        "IN roles_lambda.lambda_handler, JSON decode error: %s", e
                    )
                ...
                if not role_name or not permissions or not description:
                    return return_response(
                        400,
                        {"Error": "Role, Permissions and Description required"},
                    )
                ...
                try:
                    ...
                    if existing:
                        LOGGER.info(
                            "IN roles_lambda.lambda_handler, Role '%s' already exists", role_name
                        )
                        return return_response(400, {"Error": "Role already exists"})
                except botocore.exceptions.ClientError as e:
                    LOGGER.exception(
                        "IN roles_lambda.lambda_handler, Error checking role existence: %s", e
                    )
                ...
                try:
                    ...
                    LOGGER.info(
                        "IN roles_lambda.lambda_handler, Role '%s' created by %s", role_name, user_id
                    )
                except botocore.exceptions.ClientError as e:
                    LOGGER.exception(
                        "IN roles_lambda.lambda_handler, Error creating role: %s", e
                    )
        ...
        # ----- /roles/{role_name} Resource -----
            ...
            if method == "GET":
                LOGGER.info(
                    "IN roles_lambda.lambda_handler, Handling GET /roles/%s", role_name
                )
                try:
                    ...
                    LOGGER.info(
                        "IN roles_lambda.lambda_handler, Role '%s' not found", role_name
                    )
                    return return_response(404, {"Error": "Role not found"})
                except botocore.exceptions.ClientError as e:
                    LOGGER.exception(
                        "IN roles_lambda.lambda_handler, Error retrieving role: %s", e
                    )
                    return return_response(500, {"Error": "Failed to get role"})

            if method == "PUT":
                LOGGER.info(
                    "IN roles_lambda.lambda_handler, Handling PUT /roles/%s", role_name
                )
                body = event.get("body") or "{}"
                try:
                    data = json.loads(body)
                    LOGGER.info(
                        "IN roles_lambda.lambda_handler, Parsed update JSON for '%s'", role_name
                    )
                except json.JSONDecodeError as e:
                    LOGGER.exception(
                        "IN roles_lambda.lambda_handler, JSON decode error: %s", e
                ...
                if not permissions or not description:
                    return return_response(
                        400, {"Error": "Permissions and description are required"}
                    )
                ...
                try:
                    table.update_item(
                        ...
                        ExpressionAttributeNames={"#P": "Permissions", "#D": "Description"},
                    ...
                    LOGGER.info(
                        "IN roles_lambda.lambda_handler, Role '%s' updated by %s", role_name, user_id
                    )
                    return return_response(200, {"Message": "Role updated"})
                except botocore.exceptions.ClientError as e:
                    LOGGER.exception(
                        "IN roles_lambda.lambda_handler, Error updating role: %s", e
                    )
                    return return_response(500, {"Error": "Failed to update role"})

            if method == "DELETE":
                LOGGER.info(
                    "IN roles_lambda.lambda_handler, Handling DELETE /roles/%s", role_name
                )
                try:
                    resp = table.get_item(Key={"Role": role_name})
                    item = resp.get("Item")
                    ...
                    LOGGER.exception(
                        "IN roles_lambda.lambda_handler, Error fetching for delete: %s", e
                    )
                ...
                try:
                    ...
                    LOGGER.info(
                        "IN roles_lambda.lambda_handler, Role '%s' deleted by %s", role_name, user_id
                    )
                    return return_response(200, {"Message": "Role deleted"})
                except botocore.exceptions.ClientError as e:
                    LOGGER.exception(
                        "IN roles_lambda.lambda_handler, Error deleting role: %s", e
                    )
                    return return_response(500, {"Error": "Failed to delete role"})

        LOGGER.info(
            "IN roles_lambda.lambda_handler, No matching route for %s %s", method, resource
        )
        return return_response(404, {"Error": "Invalid resource or method"})

    except Exception as e:
        LOGGER.exception(
            "IN roles_lambda.lambda_handler, Unhandled exception: %s", e
        )
