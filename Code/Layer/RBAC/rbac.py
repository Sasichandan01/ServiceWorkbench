    ...
    if API_MAPPING is None:
        try:
            with open(path, "r", encoding="utf-8") as f:
                ...
                LOGGER.info(
                    "IN rbac.load_api_mapping, loaded API permission mapping from %s",
                    path
                )
        except (OSError, json.JSONDecodeError):
            LOGGER.exception(
                "IN rbac.load_api_mapping, failed to load API permission mapping from %s",
                path
            )
            API_MAPPING = {}
    return API_MAPPING
...
def is_user_action_valid(
    ...
    try:
        ...
        LOGGER.exception(
            "IN rbac.is_user_action_valid, DynamoDB get_item failed for role %s",
            role
        )
    ...
    if "Item" not in response:
        LOGGER.warning(
            "IN rbac.is_user_action_valid, role %r not found", role
        )
    ...
    if user_id not in users:
        LOGGER.info(
            "IN rbac.is_user_action_valid, user %r not in role %r", user_id, role
        )
    ...
    if not api_perms:
        LOGGER.info(
            "IN rbac.is_user_action_valid, no API permissions found for %s %s",
            resource, method
        )
    ...
    for perm in api_perms:
        try:
            ...
            LOGGER.info(
                "IN rbac.is_user_action_valid, checking required perm %r (rank %d)",
                perm, req_rank
            )
        except ValueError:
            LOGGER.warning(
                "IN rbac.is_user_action_valid, malformed API-perm entry: %r", perm
            )
            continue
        ...
        for rp in role_perms:
            try:
                ...
                LOGGER.info(
                    "IN rbac.is_user_action_valid, role has perm %r.%r",
                    perm_key, perm_level
                )
            except ValueError:
                LOGGER.warning(
                    "IN rbac.is_user_action_valid, malformed role-perm entry: %r", rp
                )
                continue
            ...
            if perm_key == key:
                ...
                if LEVEL_RANK.get(perm_level, 0) < req_rank:
                    ...
                    LOGGER.info(
                        "IN rbac.is_user_action_valid, insufficient level for %s", key
                    )
                break
        if not match_found:
            missing_perms.append(key)
            LOGGER.info(
                "IN rbac.is_user_action_valid, missing permission for key %s", key
            )
...
def sync_system_roles(
    ...
    if roles_mapping is None:
        try:
            with open(path, "r", encoding="utf-8") as f:
                ...
                LOGGER.info(
                    "IN rbac.sync_system_roles, loaded roles mapping from %s", path
                )
        except (OSError, json.JSONDecodeError):
            LOGGER.exception(
                "IN rbac.sync_system_roles, failed to load roles mapping from %s", path
            )
            return

    LOGGER.info(
        "IN rbac.sync_system_roles, syncing %d roles", len(roles_mapping)
    )
    ...
    for role_name, permissions in roles_mapping.items():
        ...
        try:
            LOGGER.info(
                "IN rbac.sync_system_roles, upserting role %r with perms %r",
            ...
            LOGGER.info(
                "IN rbac.sync_system_roles, role %r upserted successfully", role_name
            )
        except AttributeError:
            LOGGER.exception(
                "IN rbac.sync_system_roles, error upserting role %r", role_name
            )
            raise
